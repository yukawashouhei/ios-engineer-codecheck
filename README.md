...
**UI ブラッシュアップ（ボーナス）**
- 検索結果セルにオーナーアイコン・言語（カラードット付き）・Star 数（68.2k 形式）を表示。アイコンは非同期取得し、セル再利用時の画像取り違えを防止
- 詳細画面のアイコンを角丸に、各数値を桁区切り表示に
- ラベル色が旧式の `darkTextColor` 固定でダークモードで文字が沈む問題を `labelColor` に修正
- 検索結果が 0 件のときは「リポジトリが見つかりませんでした」を表示

**新機能（ボーナス）**
- 検索結果の総件数をセクションヘッダーに表示（`total_count` を利用）
- 詳細画面に「GitHubで見る」ボタンを追加し、リポジトリページを Safari で開けるように（`html_url` を利用）
